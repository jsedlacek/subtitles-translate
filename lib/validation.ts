...
export function validateChunk(chunk: ChunkInfo, translatedEntries: TranscriptEntry[]): void {
	...
	const expectedNumbers = new Set(chunk.translateSegments.map((s) => s.sequence));
	const actualNumbers = new Set(translatedEntries.map((e) => e.number));

	const missingNumbers = [...expectedNumbers].filter((n) => !actualNumbers.has(n));
	const extraNumbers = [...actualNumbers].filter((n) => !expectedNumbers.has(n));
...
export function validateTranslation(
	...
	const missingNumbers = [...originalNumbers].filter((n) => !translatedNumbers.has(n));
	const extraNumbers = [...translatedNumbers].filter((n) => !originalNumbers.has(n));
...
export function analyzeTranslationFailure(
	...
	if (missingNumbers.length > 0) {
		...
		if (firstMissing === undefined)
			return { insights: [], missingNumbers, extraNumbers, sequenceGaps };

		let gapStart = firstMissing;
		let gapEnd = firstMissing;
	...
	if (missingNumbers.length > 0) {
		...
		if (sequenceGaps.length === 1 && sequenceGaps[0]?.start === sequenceGaps[0]?.end) {
			const firstMissing = missingNumbers[0];
			if (firstMissing !== undefined) {
				insights.push(`Only missing segment ${firstMissing} - likely a single segment issue`);
			}
		} else if (sequenceGaps.length === 1) {
			const gap = sequenceGaps[0];
			if (!gap) return { insights, missingNumbers, extraNumbers, sequenceGaps };
			insights.push(
	...
	if (missingNumbers.length === 2 && extraNumbers.length === 0) {
		insights.push("Exactly 2 missing segments - common when LLM skips or merges segments");
	}
